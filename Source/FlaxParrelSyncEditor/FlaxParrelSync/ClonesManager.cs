...
namespace FlaxParrelSync
{
    ...
    public class ClonesManager
    {
        ...
        public static Project CreateCloneFromPath(string sourceProjectPath)
        {
            ...
            //Copy project file
            string pathToSourceProjectFilename = Path.Combine(sourceProject.projectPath, sourceProject.projectFilename);
            string pathToCloneProjectFilename = Path.Combine(cloneProject.projectPath, cloneProject.projectFilename);
            Debug.Log("Project file copy: " + sourceProject.projectFilename);
            File.Copy(pathToSourceProjectFilename, pathToCloneProjectFilename);
            
            
            //Copy Folders
            Debug.Log("Binaries copy: " + cloneProject.binariesPath);
            CopyDirectoryWithProgressBar(sourceProject.binariesPath, cloneProject.binariesPath,
                "Cloning Project Library '" + sourceProject.name + "'. ");
            Debug.Log("Cache copy: " + cloneProject.cachePath);
            CopyDirectoryWithProgressBar(sourceProject.cachePath, cloneProject.cachePath,
              "Cloning Project Packages '" + sourceProject.name + "'. ");


            //Link Folders
            Debug.Log("Content symlink: " + cloneProject.contentPath);
            LinkFolders(sourceProject.contentPath, cloneProject.contentPath);
            Debug.Log("Source symlink: " + cloneProject.sourcePath);
            LinkFolders(sourceProject.sourcePath, cloneProject.sourcePath);

            //Optional Link Folders

            var preferences = PluginManager.GetPlugin<FlaxParrelSyncEditor>().GetPreferences();
            
            foreach (var path in preferences.AdditionalSymbolicLinkFolders)
            {
        ...
        public static bool IsCloneProjectRunning(string projectPath)
        {
            ...
            //Determine whether it is opened in another instance by checking the LockFile
            string LockFilePath = new string[] { projectPath, "Lockfile" }
                .Aggregate(Path.Combine);

            var preferences = PluginManager.GetPlugin<FlaxParrelSyncEditor>().GetPreferences();
            
            //Windows editor will lock "Lockfile" file when project is being opened.
            //Sometime, for instance: windows editor crash, the "Lockfile" will not be deleted even the project
            //isn't being opened, so a check to the "Lockfile" lock status may be necessary.
            if (preferences.AlsoCheckLockFileStaPref)
                return File.Exists(LockFilePath) && FileUtilities.IsFileLocked(LockFilePath);
            else
        ...
        public static void DeleteClone(string cloneProjectPath)
        {
            ...
            Directory.Delete(cloneProjectPath, true);
        ...
        public static string GetArgument()
        {
            ...
            if (IsClone())
            {
                ...
                {
                    argument = File.ReadAllText(argumentFilePath, Encoding.UTF8);
                }
            }
